...
if ! rg -q 'VERSION_NAME = "0\.6\.12"' "$core_root/BuildIdentity.java" \
    || ! rg -q 'VERSION_CODE = 79' "$core_root/BuildIdentity.java" \
    || ! rg -q 'CODEX_RUNTIME_VERSION = "0\.148\.1"' "$core_root/BuildIdentity.java" \
    || ! rg -q 'android:versionName="0\.6\.12"' "$manifest" \
    || ! rg -q 'android:versionCode="79"' "$manifest" \
    || ! rg -q 'APP_VERSION="0\.6\.12"' "$apk_builder" \
    || ! rg -q 'VERSION_CODE="79"' "$apk_builder" \
  ...
  echo "The 0.6.12 / Codex 0.148.1 identity is inconsistent." >&2
  exit 1
fi
