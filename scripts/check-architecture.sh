...
if ! rg -q 'VERSION_NAME = "0\.6\.11"' "$core_root/BuildIdentity.java" \
    || ! rg -q 'VERSION_CODE = 78' "$core_root/BuildIdentity.java" \
    || ! rg -q 'CODEX_RUNTIME_VERSION = "0\.148\.1"' "$core_root/BuildIdentity.java" \
    || ! rg -q 'android:versionName="0\.6\.11"' "$manifest" \
    || ! rg -q 'android:versionCode="78"' "$manifest" \
    || ! rg -q 'APP_VERSION="0\.6\.11"' "$apk_builder" \
    || ! rg -q 'VERSION_CODE="78"' "$apk_builder" \
  ...
  echo "The 0.6.11 / Codex 0.148.1 identity is inconsistent." >&2
  exit 1
fi
