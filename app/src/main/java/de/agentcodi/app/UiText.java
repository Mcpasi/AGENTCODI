...
final class UiText {
    ...
    private static int coreStatusResource(String raw) {
        ...
        if ("Review-Modus ist in dieser Runtime nicht verfügbar.".equals(raw)) return R.string.core_review_unavailable;
        if ("Öffne zuerst einen Chat für den Review-Modus.".equals(raw)) return R.string.core_review_open_chat;
        if ("Review-Anweisungen müssen 1 bis 32768 Zeichen enthalten.".equals(raw)) return R.string.core_review_instruction_length;
        if ("Review wird gestartet.".equals(raw)) return R.string.core_review_starting;
        if ("Codex prüft den Workspace.".equals(raw)) return R.string.core_review_working;
        if ("Review abgeschlossen.".equals(raw)) return R.string.core_review_complete;
        if ("Der aktive Chat hat sich vor dem Review geändert.".equals(raw)) return R.string.core_review_chat_changed;
        if ("Der App-Server hat den Review nicht korrekt bestätigt.".equals(raw)) return R.string.core_review_response_invalid;
        if ("Der native Review-Ablauf ist fehlgeschlagen.".equals(raw)) return R.string.core_review_protocol_failed;
        ...
        if ("Der aktive Turn wird noch sicher korreliert.".equals(raw)) return R.string.core_turn_correlation_pending;
        if ("Der Turn-Stopp wird noch bestätigt.".equals(raw)) return R.string.core_turn_stop_pending;
